-- "addons\\rised_tech\\lua\\weapons\\rebel_turret_placer.lua"
...
function SWEP:HealthDrop()
	if SERVER then
		...
		if ply:Team() != TEAM_REBELENGINEER then DarkRP.notify(ply, 0, 3, "Вы не владеете навыком установки турелей!") return end
		...
		if IsValid( support ) then
			...
			if IsValid( support.turret ) then
				...
				support.turret:SetNWString("Entity_Fraction", "Rebel")
				support.turret:SetMaterial("models/props_citizen_tech/steam_engine001")
				
				support.turret:EmitSound("npc/turret_floor/deploy.wav")
				for k,v in pairs(player.GetAll()) do 
					if !v:isCP() and !GAMEMODE.ZombieJobs[ply:Team()] then
			...
			if ply:GetActiveWeapon() == self then
				...
				if distply > 100 or !eyetrace.HitWorld then
					...
					if !self.viewturret.WellPlaced then
						...
						self.viewturret:SetMaterial("models/props_citizen_tech/steam_engine001")
						self.viewturret.WellPlaced = true
					end
				end
			else
