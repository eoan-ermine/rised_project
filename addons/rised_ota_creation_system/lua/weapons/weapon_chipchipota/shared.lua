-- "addons\\rised_ota_creation_system\\lua\\weapons\\weapon_chipchipota\\shared.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
AddCSLuaFile()

if CLIENT then
    SWEP.PrintName = "Чипирование в ОТА"
    SWEP.Slot = 2
    SWEP.SlotPos = 10
    SWEP.DrawAmmo = false
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		if ent:GetDriver():getDarkRPVar("money") < 25 then return end
		
		--if ent:GetDriver():GetModel() == "models/beta stalker/beta_stalker.mdl" or ent:GetDriver():GetModel() == "models/player/soldier_stripped.mdl" then return end
		...
		if ent:GetDriver():IsPlayer() then
			...
			self:SetEndCheckTime(CurTime() + 60)
...
function SWEP:Succeed()
	if SERVER then
		...
		if !IsValid(trEntity:GetDriver()) then return end
		
		if trEntity:GetDriver():getDarkRPVar("money") < 25 then return end
		
		self.Owner:addMoney(2)
		trEntity:GetDriver():addMoney(-5)
		
		DarkRP.notify(self.Owner,0,5,"Вы заработали с операции 5 Т.")
		DarkRP.notify(trEntity:GetDriver(),0,5,"Вы потратили на операцию 25 Т.")
		
		trEntity:GetDriver():SetModel("models/player/soldier_stripped.mdl")
		trEntity:GetDriver():EmitSound("npc/stalker/breathing3.wav")
		trEntity:GetDriver():StripWeapons()
		trEntity:GetDriver():Lock()
		
		timer.Simple(1, function()

			if trEntity:GetDriver():IsPlayer() then
				local ota = trEntity:GetDriver()
				
				trEntity:GetDriver():UnLock()
				trEntity:GetDriver():ExitVehicle()
				
				if ota:GetNWBool("rhc_cuffed", false) then
					ota:RHCRestrain(self.Owner)
				end
				
				ota:changeTeam(self.Owner:GetNWInt("JobToDo"), true)
				ota:ScreenFade(SCREENFADE.IN, color_black, 10, 3)
				
				AddTaskExperience(self.Owner, "Создание ОТА")
				
				self.Owner:SetNWInt("JobToDo", nil)

				timer.Simple(1, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
				timer.Simple(2, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
				timer.Simple(3, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
				timer.Simple(4, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
				timer.Simple(5, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
				timer.Simple(6, function() ota:PrintMessage( HUD_PRINTCENTER, "Ваше тело подверглось тяжелым техно-биологическим изменениям..." ) end)
			end
		
			self.Owner:StripWeapon("weapon_chipchipota")
			
		end)
		
		self.Owner:SetNWInt("Entity_Timer", CurTime() + 1)
	end
end
...
function SWEP:DrawHUD()
    if self:GetIsWeaponChecking() and self:GetEndCheckTime() ~= 0 then
        ...
        draw.DrawNonParsedSimpleText("Чипирование в ОТА" .. self.Dots, "Trebuchet18", w / 2, y + height / 4, Color(255, 195, 87, 255), 1, 1)
    end
end
