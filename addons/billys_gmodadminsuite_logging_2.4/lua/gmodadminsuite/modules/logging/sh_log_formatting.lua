-- "addons\\billys_gmodadminsuite_logging_2.4\\lua\\gmodadminsuite\\modules\\logging\\sh_log_formatting.lua"
...
function GAS.Logging:ProcessReplacement(replacement, allow_color, markdown_format, victim_account_id, instigator_account_id)
	if (replacement[1] == GAS.Logging.FORMAT_PLAYER) then
		...
		elseif (tonumber(replacement[2])) then
			...
			if (IsValid(ply)) then
				ply_replacement = EncapsulateColor(team.GetColor(ply:Team()), ply:SteamName() .. " (" .. ply:SteamID() .. ")", allow_color)
			elseif (replacement[3] ~= nil and replacement[3][1] ~= nil) then
				if replacement[3][8] ~= nil then
					ply_replacement = EncapsulateColor(GAS.Logging.LogFormattingSettings.Colors.Unavailable, replacement[3][1] .. " (" .. replacement[3][8] .. ")", allow_color)
				else
