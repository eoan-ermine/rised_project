-- "addons\\billys_gmodadminsuite_logging_2.4\\lua\\vgui\\gas_logging_damagelog.lua"
...
function PANEL:Setup(data, allow_click)
	...
	if (IsValid(instigator)) then
		self.InstigatorName:SetText(instigator:SteamName())
	else
	...
	if (IsValid(victim)) then
		self.VictimName:SetText(victim:SteamName())
	else
