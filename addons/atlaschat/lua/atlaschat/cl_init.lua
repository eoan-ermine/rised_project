-- "addons\\atlaschat\\lua\\atlaschat\\cl_init.lua"
...
function atlaschat.FixInvalidFont()
	...
	if (!ok) then
		atlaschat.font:SetString("atlaschat.courier.new")
...
hook.Add("InitPostEntity", "atlaschat.CreateChatbox", function()
	...
	if (!theme) then
		...
		atlaschat.themeConfig:SetString("blur")
	end
...
hook.Add("DrawOverlay", "atlaschat.DrawCopiedNotify", function()
	for i = 1, #copiedNotify do
		...
		if (data) then
			draw.SimpleText("Текст скопирован!", "atlaschat.courier.new", data.x, data.y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
		...
		function child:OnMousePressed(code)
			if (code == MOUSE_RIGHT) then
				local menu = DermaMenu()
					menu:AddOption("Скопировать", function()
						local clipboardText = ""
...
function atlaschat.GenericLabel()
	...
	function label:Paint(w, h)
		local shadow, text, font, pcall = shadow, self:GetText(), self:GetFont(), pcall
