-- "addons\\atlaschat\\lua\\atlaschat\\themes\\default.lua"
...
atlaschat.font 					= atlaschat.config.New("Шрифт чата", 						"font", 			"atlaschat.courier.new",true)
atlaschat.chatSound				= atlaschat.config.New("Проигрывать звук чата", 			"chat_sound", 		true, 					true)
atlaschat.fadetime 				= atlaschat.config.New("Скорость исчезновения",				"fadetime", 		12, 					true)
atlaschat.maxHistory 			= atlaschat.config.New("Маскимум истории чата", 			"max_history", 		25, 					true)
...
function theme:Initialize()
	...
	self.mainChat = panel.chatrooms:AddChatroom("")
	self.mainChat:SetList(panel.list)
	self.mainChat:OnMousePressed(MOUSE_LEFT)
	self.mainChat.Paint = function(self, w, h) end
	
	self.settingsIcon = panel:AddIcon("atlaschat/settings.png", function() atlaschat.theme.Call("ToggleConfigPanel") end, "Настройки")
			...
			if (info.name == "font") then
				...
				function panel:OnMousePressed()
					...
					for i = 1, #fonts do
						...
						local text = "Вот такой шрифт \"" .. data[1] .. "\""
...
function theme:ToggleConfigPanel()
	if (!ValidPanel(self.config)) then
		...
		self.config:AddButton("Сбросить настройки", nil, function()
		...
		self.config:AddButton("Включить/выключить OOC чат", nil, function()
			net.Start("Rised_Chat_OOC_Disable")
			net.SendToServer()
			chat.PlaySound()
		end)
		
		if (LocalPlayer():IsSuperAdmin()) then
			self.config:AddButton("Сбросить настройки чата у всех", true, function()
				net.Start("atlaschat.rqclrcfg")
				net.SendToServer()
			end)
...
function theme:OnToggle(show)
	...
	if (self.panel.entry.key) then
		...
		if (self.panel.team) then
			self.panel.prefix:SetPrefix("КОМАНДА")
		else
			self.panel.prefix:SetPrefix("НАПИСАТЬ")
		end
	end
...
function theme:ParseExpressionsTest(data, panel, player, title)
...
function theme:ParseData(data, list, isTitle)
	...
	for i = 1, #data do
		...
		if (type == "Player") then
			...
			--self:ParseExpressionsTest(titleData, parseBase, value, true)
