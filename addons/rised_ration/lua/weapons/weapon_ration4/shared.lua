-- "addons\\rised_ration\\lua\\weapons\\weapon_ration4\\shared.lua"
...
function SWEP:PrimaryAttack()
	if SERVER then
		if self.Owner:getDarkRPVar("Energy") <= 85 then
			self.Owner:setDarkRPVar("Energy", self.Owner:getDarkRPVar("Energy") + 5)
		elseif self.Owner:getDarkRPVar("Energy") <= 100 then
			self.Owner:setDarkRPVar("Energy", 100)
		end
			
		-- Настроение --
		hook.Call("playerMoodChanged", GAMEMODE, self.Owner, -3)
