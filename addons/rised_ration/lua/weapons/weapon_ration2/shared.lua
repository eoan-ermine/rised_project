-- "addons\\rised_ration\\lua\\weapons\\weapon_ration2\\shared.lua"
...
function SWEP:PrimaryAttack()
	if SERVER then
		if self.Owner:getDarkRPVar("Energy") <= 65 then
			self.Owner:setDarkRPVar("Energy", self.Owner:getDarkRPVar("Energy") + 15)
		elseif self.Owner:getDarkRPVar("Energy") <= 100 then
			self.Owner:setDarkRPVar("Energy", 100)
		end
		
		local add_exp = (((self.Owner.Player_LevelCommon or 0) - 1) * 200 + 1000) * math.Round(math.Rand(0.02, 0.04), 2)
		AddExperienceWithCD(self.Owner, add_exp, "Common", "RationEating", 120)
			
		-- Настроение --
		hook.Call("playerMoodChanged", GAMEMODE, self.Owner, -1)
