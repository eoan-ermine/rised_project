-- "addons\\darkrpmodification\\lua\\weapons\\weapon_weapons_jeff\\shared.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
AddCSLuaFile("shared.lua")

if CLIENT then
SWEP.PrintName = "Зараженный"
...
function SWEP:Deploy()
	...
	--self:EmitSound("npc/zombie_poison/pz_breathe_loop1.wav", 35)
...
function SWEP:Holster()
	...
	self:Playermodel()
	--self:StopSound("npc/zombie_poison/pz_breathe_loop1.wav")
	end
	return true;
end
	...
	timer.Simple(4.2, function(wep)
		if SERVER then
			if IsValid(self) then
				...
				net.WriteInt(1, 32)
				net.Broadcast()
				self.Owner:StopSound("npc/zombie/moan_loop1.wav")
				self.Owner:StopSound("npc/zombie/moan_loop2.wav")
				self.Owner:StopSound("npc/zombie/moan_loop3.wav")
				self.Owner:StopSound("npc/zombie/moan_loop4.wav")
			end
		end
	end)
	
    if SERVER and not CLIENT then
        self.Owner:EmitSound("npc/zombie/moan_loop"..math.random(1,4)..".wav")
    end
