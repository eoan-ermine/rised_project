-- "addons\\darkrpmodification\\lua\\weapons\\weapon_mpfinfestationup\\shared.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal

AddCSLuaFile()

if CLIENT then
SWEP.PrintName			= "Повышение IC"
SWEP.Slot				= 4
SWEP.SlotPos			= 39
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		if trEntity:IsPlayer() and !GAMEMODE.ZombieJobs[trEntity:Team()] then
			trEntity:SetNWBool("MPF_IC_PERMISSION", true)
			DarkRP.notify(self.Owner,2,7,"Вы выдали ".. trEntity:Nick() .. " доступ к Infectation Control.")
			DarkRP.notify(trEntity,0,7,"Вам выдали доступ к Infectation Control!")
		end
...
function SWEP:SecondaryAttack()	
	if SERVER then
		...
		if trEntity:IsPlayer() and trEntity:GetNWBool("MPF_IC_PERMISSION") == true then
			trEntity:SetNWBool("MPF_IC_PERMISSION", false)
			DarkRP.notify(self.Owner,2,7,"Вы забрали у ".. trEntity:Nick() .. " доступ к Infectation Control.")
			DarkRP.notify(trEntity,0,7,"У вас забрали доступ к Infectation Control!")
		end
