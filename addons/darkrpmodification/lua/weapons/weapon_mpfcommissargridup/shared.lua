-- "addons\\darkrpmodification\\lua\\weapons\\weapon_mpfcommissargridup\\shared.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal

AddCSLuaFile()

if CLIENT then
SWEP.PrintName			= "Повышение GRID"
SWEP.Slot				= 4
SWEP.SlotPos			= 33
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		if trEntity:IsPlayer() and GAMEMODE.MetropoliceJobs[trEntity:Team()] then
			trEntity:SetNWBool("MPF_GRID_PERMISSION", true)
			DarkRP.notify(self.Owner,2,7,"Вы выдали ".. trEntity:Nick() .. " доступ к GRID.")
			DarkRP.notify(trEntity,0,7,"Вам выдали доступ к GRID!")
		end
...
function SWEP:SecondaryAttack()	
	if SERVER then
		...
		if trEntity:IsPlayer() and trEntity:GetNWBool("MPF_GRID_PERMISSION") == true then
			trEntity:SetNWBool("MPF_GRID_PERMISSION", false)
			DarkRP.notify(self.Owner,2,7,"Вы забрали у ".. trEntity:Nick() .. " доступ к GRID.")
			DarkRP.notify(trEntity,0,7,"У вас забрали доступ к GRID!")
		end
