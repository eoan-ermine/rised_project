-- "addons\\darkrpmodification\\lua\\weapons\\weapon_mpfcuratorup\\shared.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal

AddCSLuaFile()

if CLIENT then
SWEP.PrintName			= "Разрешение на RECRUIT"
SWEP.Slot				= 4
SWEP.SlotPos			= 30
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		if trEntity:IsPlayer() and !trEntity:isCP() and !GAMEMODE.Rebels[trEntity:Team()] then
			if trEntity:HasWeapon("medicalcard") then
				if tonumber(trEntity:GetNWInt("Player_CombineRank")) > 2 then
					trEntity:SetNWBool("MPF_RECRUIT_PERMISSION", true)
					DarkRP.notify(self.Owner,2,7,"Вы выдали ".. trEntity:Nick() .. " разрешение на вступление в ГО.")
					DarkRP.notify(trEntity,0,7,"Вам выдали разрешение на вступление в ГО!")
				else
					DarkRP.notify(self.Owner,0,7,"У гражданского недостаточно пунктов повышения!")
				end
			else
				DarkRP.notify(self.Owner,0,7,"У гражданского нет медицинской карты!")
			end
		end
...
function SWEP:SecondaryAttack()	
	if SERVER then
		...
		if trEntity:IsPlayer() and trEntity:GetNWBool("MPF_RECRUIT_PERMISSION") == true then
			trEntity:SetNWBool("MPF_RECRUIT_PERMISSION", false)
			DarkRP.notify(self.Owner,2,7,"Вы забрали у ".. trEntity:Nick() .. " разрешение на вступление в ГО.")
			DarkRP.notify(trEntity,0,7,"У вас забрали разрешение на вступление в ГО!")
		end
