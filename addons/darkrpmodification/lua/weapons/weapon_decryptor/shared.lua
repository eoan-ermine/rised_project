-- "addons\\darkrpmodification\\lua\\weapons\\weapon_decryptor\\shared.lua"
...
function SWEP:PrimaryAttack()
	if SERVER then
		...
		local vort = trace.Entity
		
		if not IsValid(vort) or vort:GetPos():DistToSqr(self:GetOwner():GetPos()) > 35000 then
			return
		end
		
		if not IsValid(vort) then return end
		if vort:Team() != TEAM_VORTIGAUNTSLAVE then return end
		
		if vort:IsPlayer() then
...
function SWEP:Succeed()
	if SERVER then
		...
		local vort = self.Owner:GetEyeTrace().Entity
    	local Distance = self.Owner:EyePos():Distance(vort:GetPos());
		...
		timer.Simple(1, function()
			...
			if vort:IsPlayer() then
				...
				if vort:GetNWBool("rhc_cuffed", false) then
					vort:RHCRestrain(self.Owner)
				end
				
				vort:changeTeam(TEAM_VORTIGAUNT, true)
				vort:ScreenFade(SCREENFADE.IN, color_black, 10, 3)
				
				timer.Simple(1, function() vort:PrintMessage( HUD_PRINTCENTER, "Вы были освобождены от оков Альянса и вы вновь можете использовать силу..." ) end)
			end
...
function SWEP:Think()
	if SERVER then
		if self:GetIsWeaponChecking() and self:GetEndCheckTime() ~= 0 then
			...
			local trace = self:GetOwner():GetEyeTrace()
			self:GetOwner():LagCompensation(false)
			if not IsValid(trace.Entity) or trace.HitPos:DistToSqr(self:GetOwner():GetShootPos()) > 35000 or trace.Entity:Team() != TEAM_VORTIGAUNTSLAVE then
				self:Fail()
			end
		...
		if self:GetNextSoundTime() ~= 0 and CurTime() >= self:GetNextSoundTime() then
			...
			if self:GetIsWeaponChecking() then
				self:SetNextSoundTime(CurTime() + 3)
				trace.Entity:ScreenFade(SCREENFADE.IN, Color( 255, 0, 0, 255 ), 0.8, 0)
				if math.random(1,2) == 1 then
					trace.Entity:EmitSound("buttons/blip1.wav", 70)
				else
					trace.Entity:EmitSound("npc/turret_floor/ping.wav", 70)
				end
			else
				self:SetNextSoundTime(0)
				trace.Entity:ScreenFade(SCREENFADE.IN, Color( 255, 0, 0, 255 ), 0.8, 0)
...
function SWEP:DrawHUD()
    if self:GetIsWeaponChecking() and self:GetEndCheckTime() ~= 0 then
        ...
        draw.DrawNonParsedSimpleText("Декриптинг" .. self.Dots, "Trebuchet18", w / 2, y + height / 4, Color(255, 195, 87, 255), 1, 1)
    end
end
...
function SWEP:DrawWorldModel()
	...
	if boneindex then	
		...
		local offset = HAng:Right() * 4 + HAng:Forward() * 7 + HAng:Up() * 0.518

		HAng:RotateAroundAxis(HAng:Right(), 90)
		HAng:RotateAroundAxis(HAng:Forward(),  80)
