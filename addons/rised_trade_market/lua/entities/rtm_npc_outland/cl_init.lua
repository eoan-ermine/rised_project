-- "addons\\rised_trade_market\\lua\\entities\\rtm_npc_outland\\cl_init.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
include("shared.lua")

net.Receive("TradeNPC.Outland.OpenMenu", function()
	TradeNPCOutlandMenuOpen()
end)

function TradeNPCOutlandMenuOpen()
	...
	frame.Paint = function(self, w, h)
		...
		draw.SimpleText("Независимый торговец", "marske6", w/2, ScrH()/2 - 310, Color(255, 255, 255, 255), 1, 2)
	...
	local i1 = GetGlobalInt("TradeNPC.Outland_Item_01")
	local i2 = GetGlobalInt("TradeNPC.Outland_Item_02")
	local i3 = GetGlobalInt("TradeNPC.Outland_Item_03")
	local i4 = GetGlobalInt("TradeNPC.Outland_Item_04")
	local i5 = GetGlobalInt("TradeNPC.Outland_Item_05")
	...
	for i=1, RISED.Config.Trade.Outland.MaxItems do
		items_to_trade[i] = RISED.Config.Trade.Outland.Items[GetGlobalInt("TradeNPC.Outland_Item_" .. i)]
	end
	...
	for k,v in pairs(items_to_trade) do
		...
		buttonAdd.OnMousePressed = function(self)
			...
			net.Start("TradeNPC.Outland.Server")
			net.WriteInt(k,10)
			net.SendToServer()
		end
	...
	for k,v in pairs(items_to_trade) do
		...
		if v.ItemCategory != "Запчасти" then continue end
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 500 then
		if (self:GetDTInt(1) == 0) then
			cam.Start3D2D(pos + ang:Up()*0, Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.07)
				...
				draw.SimpleText("Независимый торговец", "Trebuchet18", -20, -1075, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
