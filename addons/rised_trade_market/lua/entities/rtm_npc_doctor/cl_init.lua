-- "addons\\rised_trade_market\\lua\\entities\\rtm_npc_doctor\\cl_init.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
include("shared.lua")

net.Receive("TradeNPC.Doctor.OpenMenu", function()
	TradeNPCDoctorMenuOpen()
end)

function TradeNPCDoctorMenuOpen()
	...
	frame.Paint = function(self, w, h)
		...
		draw.SimpleText("Леонард Веррен", "marske8", w/2, ScrH()/2 - 355, Color(255, 255, 255, 255), 1, 2)
		draw.RoundedBox(0, w/2 - 150, ScrH()/2 - 320, 300, 1, Color(255, 255, 255, 255));
		draw.SimpleText("Врач", "marske6", w/2, ScrH()/2 - 310, Color(255, 255, 255, 255), 1, 2)
	...
	local i1 = GetGlobalInt("TradeNPC.Doctor_Item_01")
	local i2 = GetGlobalInt("TradeNPC.Doctor_Item_02")
	local i3 = GetGlobalInt("TradeNPC.Doctor_Item_03")
	local i4 = GetGlobalInt("TradeNPC.Doctor_Item_04")
	local i5 = GetGlobalInt("TradeNPC.Doctor_Item_05")
	...
	for i=1, RISED.Config.Trade.Doctor.MaxItems do
		items_to_trade[i] = RISED.Config.Trade.Doctor.Items[GetGlobalInt("TradeNPC.Doctor_Item_" .. i)]
	end
	...
	for k,v in pairs(items_to_trade) do
		...
		if v.ItemCategory != "Первая помощь" then continue end
		...
		buttonAdd.Desc = v.ItemDescription
		...
		buttonAdd.Paint = function(self, w, h)
			...
			draw.SimpleText(self.Desc, "marske4", 40, h/1.25, Color(155, 155, 155, buttonAdd.alpha), 0, 1);
		end;
		...
		buttonAdd.OnMousePressed = function(self)
			...
			net.Start("TradeNPC.Doctor.Server")
			net.WriteInt(k,10)
			net.SendToServer()
		end
	...
	for k,v in pairs(items_to_trade) do
		...
		if v.ItemCategory != "Терапия" then continue end
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 500 then
		if (self:GetDTInt(1) == 0) then
			cam.Start3D2D(pos + ang:Up()*0, Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.07)
				...
				draw.SimpleText("Леонард Веррен", "marske6", -20, -1100, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
				surface.SetDrawColor( 255, 255, 255, 50 )
				surface.DrawLine( -90, -1085, 50, -1085 )
				draw.SimpleText("Врач", "Trebuchet18", -20, -1075, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
