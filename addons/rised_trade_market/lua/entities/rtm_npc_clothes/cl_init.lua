-- "addons\\rised_trade_market\\lua\\entities\\rtm_npc_clothes\\cl_init.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
include("shared.lua")

net.Receive("TradeNPC.Clothes.OpenMenu", function()
	TradeNPCClothesMenuOpen()
end)

function TradeNPCClothesMenuOpen()
	...
	if RISED.Config.Trade.Clothes.IsRandomRotation then
		for i=1, RISED.Config.Trade.Clothes.MaxItems do
			items_to_trade[i] = RISED.Config.Trade.Clothes.Items[GetGlobalInt("TradeNPC.Clothes_Item_" .. i)]
		end
	else
	...
	for k,v in pairs(items_to_trade) do
		...
		buttonAdd.OnMousePressed = function(self)
			...
			net.Start("TradeNPC.Clothes.Server")
			net.WriteInt(k,10)
			net.SendToServer()
		end
		...
		itemModel.Entity:SetSkin( v.ClothSkin )
