...
local function addEntityCommands(tblEnt)
    ...
    local function buythis(ply, args)
		...
		if tblEnt.workstatus != nil then
			if ply:GetNWString("Player_WorkStatus") == tblEnt.workstatus then
				needswepblock = false
			end
		end
		
        if type(tblEnt.allowed) == "table" and not table.HasValue(tblEnt.allowed, ply:Team()) and needswepblock then
